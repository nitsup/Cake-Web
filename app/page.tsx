...
const featuredCakes = [
  { name: "The Layered Classic", category: "Signature", description: "A quiet, generous centerpiece waiting for its final flourish." },
  { name: "Garden Party", category: "Seasonal", description: "Soft layers and a little room for celebration." },
  { name: "Sunday Chocolate", category: "Comfort", description: "Rich, familiar, and made for a slow afternoon." },
];
    ...
    <div className="overflow-hidden">
      <section className="hero-band">
        <div className="container grid gap-10 py-16 md:grid-cols-[.9fr_1.1fr] md:items-center md:py-24 lg:py-28">
          <Reveal><p className="eyebrow">A temporary storefront</p><h1 className="display-heading mt-5 max-w-xl">Cakes with a little more feeling.</h1><p className="mt-6 max-w-lg text-lg leading-8 text-muted-foreground">Thoughtful cakes for the tables, milestones, and ordinary Tuesdays that deserve something lovely.</p><div className="mt-8 flex flex-wrap items-center gap-3"><Link href="#featured" className="button button--primary">See the collection <ArrowRight size={17} aria-hidden="true" /></Link><Link href="#story" className="button button--secondary">Our approach</Link></div></Reveal>
      ...
      <section id="featured" className="container py-16 md:py-24"><div className="flex flex-col justify-between gap-6 md:flex-row md:items-end"><SectionHeading eyebrow="A first look" title="Made for the middle of the table." description="A placeholder collection to establish the rhythm of the storefront while the final cakes and photography are prepared." /><Link href="#contact" className="inline-flex items-center gap-2 text-sm font-bold text-accent">Browse all <ArrowRight size={16} aria-hidden="true" /></Link></div><div className="mt-10 grid gap-5 md:grid-cols-2 lg:grid-cols-3">{featuredCakes.map((cake, index) => <Reveal key={cake.name} delay={index * 0.08}><CakeCard {...cake} /></Reveal>)}</div></section>
